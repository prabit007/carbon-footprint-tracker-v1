      ...
      <section className="border-b border-border bg-white pt-14 pb-16">
        <div className="mx-auto grid w-full max-w-[1080px] grid-cols-[1.2fr_0.8fr] items-center gap-12 px-6 max-[860px]:grid-cols-1 max-[860px]:text-center max-[640px]:px-4">
          <div>
            ...
            <button
              className="mt-[26px] inline-flex cursor-pointer items-center justify-center gap-2 rounded border border-green bg-green px-[18px] py-2.5 text-[0.9rem] font-semibold text-white hover:bg-green-dark"
              onClick={onStart}
            >
              Calculate my Carbon Footprint
            </button>
            <p className="mt-3 text-[0.8rem] text-muted">Takes about a minutes. Nothing leaves your browser.</p>
          </div>
          <div className="mx-auto w-full max-w-[280px] max-[860px]:order-first max-[860px]:max-w-[220px]">
            <img src={heroVisual.src} alt={heroVisual.alt} className="h-auto w-full" />
          </div>
        </div>
      </section>
      <StatsStrip />
